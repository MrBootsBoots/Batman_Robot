...
from batman import dispatcher, BAN_STICKER
from batman.modules.disable import DisableAbleCommandHandler
from batman.modules.helper_funcs.chat_status import is_user_admin, bot_admin, user_admin_no_reply, user_admin, \
    can_restrict
from batman.modules.helper_funcs.extraction import extract_text, extract_user_and_text, extract_user
from batman.modules.helper_funcs.filters import CustomFilters
from batman.modules.helper_funcs.misc import split_message
from batman.modules.helper_funcs.string_handling import split_quotes
from batman.modules.log_channel import loggable
from batman.modules.sql import warns_sql as sql
...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    ...
    if num_warns >= limit:
        ...
        reply = "{} has {}/{} warnings... be careful!".format(mention_html(user.id, user.first_name), num_warns,
                                                             limit)
        if reason:
    ...
    if match:
        ...
        if res:
            update.effective_message.edit_text(
                "Congrats, Warn removed by {}.".format(mention_html(user.id, user.first_name)),
